...
@Composable
fun VideoContent() {
    var seek by rememberSaveable { mutableStateOf(0L) }
    val dataSourceFactory = DefaultHttpDataSource.Factory()
    val dashSource = DashMediaSource.Factory(dataSourceFactory)
        .createMediaSource(
            MediaItem.fromUri("https://livesim.dashif.org/livesim/chunkdur_1/ato_7/testpic4_8s/Manifest.mpd")
        )
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        val exoPlayer =
            ExoPlayer.Builder(context).build()
                .also { exoPlayer ->
//                    val mediaItem =
//                        MediaItem.Builder().setUri(Uri.parse("asset:///sample.mp4")).build()
//                    exoPlayer.setMediaItem(mediaItem)
                    exoPlayer.setMediaSource(dashSource)
                    exoPlayer.prepare()
        ...
        Button(onClick = {
            val downloadRequest = DownloadRequest.Builder("0", "".toUri()).build()
        ...
        DisposableEffect(Unit) {
            onDispose {
                seek = exoPlayer.currentPosition
                exoPlayer.release()
            }
        }
    }
